...
class FPEnergyTrainer(nn.Module):
    ...
    here gate is trained in parallel with main model"""
    def __init__(
            self,
        ...
        self.gate_entropy = []  # how confident is the gate?
        self.gate_prob_distribution = []  # distribution of gate probabilities
        self.gate_flip_rate = []
        self.fp_residual_stats = []  # track residual magnitudes over time
        self.fp_residual_vs_time = []
    ...
    def forward(self):
        ...
        for epoch in range(self.epochs):
            ...
            epoch_losses, epoch_gate_losses = self.train_batch_with_gate_updates(pbar, epoch)
            mean_loss = sum(epoch_losses) / len(epoch_losses)
            mean_gate_loss = sum(epoch_gate_losses) / len(epoch_gate_losses) if epoch_gate_losses else 0
            self.losses.append(mean_loss)
            self.gate_losses.append(mean_gate_loss)
            if (epoch + 1) % self.log_freq == 0:
                lr = self.optimizer.param_groups[0]['lr']
                print(f"Epoch: {epoch + 1}/{self.epochs} | LR: {lr:.2e} | "
                      f"Train Loss: {mean_loss:.4f} | Gate Loss: {mean_gate_loss:.4f}")
    ...
    def train_batch_with_gate_updates(self, pbar, epoch):
        """train energy network and gate together with interleaved updates"""
        epoch_losses = []
        epoch_gate_losses = []
        gate_update_freq = 3 if epoch < 10 else 10
        for step, batch in enumerate(pbar):
            ...
            # periodically update gate
            if step % gate_update_freq == 0:
                gate_loss = self.gate_step(
                    x0, atom_features, edge_index, batch_idx, num_molecules
                )
                gate_loss.backward()
                ...
                pbar.set_postfix({
                    'energy_loss': f'{step_loss.item() * self.grad_acc:.4f}',
        ...
        if self.global_step >= self.warmup_steps:
            self.scheduler.step()
        return epoch_losses, epoch_gate_losses
    ...
    def train_step(self, x0: torch.Tensor, atom_features: torch.Tensor, edge_index: torch.Tensor,
        ...
        dsm_loss = self.lambda_t(t_atom.mean().item()) * self.dsm_loss(pred_score, true_score, var) # weighted dsm-loss
        with torch.no_grad():
        ...
        if apply_fp_mask.any():
            ...
            #    print(f'|r1|: {r1.abs().mean():.4f}, |r2|: {r2.abs().mean():.4f}, '
            #          f'gate_rate: {apply_fp_mask.float().mean():.2%}')
            #---------------------------------------------------------------------
    ...
    def gate_step(self, x0: torch.Tensor, atom_features: torch.Tensor, edge_index: torch.Tensor,
        ...
        #xt = xt.detach().requires_grad_(True)
        #logp = self.energy_net(xt, atom_features, edge_index, t_atom, batch_idx)
        with torch.no_grad():  # Don't backprop through energy network during gate update
            logp = self.energy_net(xt, atom_features, edge_index, t_atom, batch_idx)
        #pred_score = self.score_fn(logp, xt)
        xt_for_score = xt.requires_grad_(True)
        logp_for_score = self.energy_net(xt_for_score, atom_features, edge_index, t_atom, batch_idx)
        pred_score = self.score_fn(logp_for_score, xt_for_score)
        with self.freeze_params(self.energy_net):
            r = self.fp_residual(
    ...
    def sample_time(self, batch_size: int, eps: float = 1e-3) -> torch.Tensor:
        """sample time from [eps, 1-eps] instead of [0, 1]
        this avoids the extremes where variance → 0 or variance → 1"""
