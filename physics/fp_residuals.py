...
def mb_heavy_fp_residual(energy_net: nn.Module, x: torch.Tensor, t: torch.Tensor, scheduler: nn.Module,
    ...
    finite-difference approximations for spatial and temporal derivatives"""
    if seed is not None:
        gen = torch.Generator(device=x.device)
        gen.manual_seed(seed)
    else:
    ...
    x_minus = (x - v).requires_grad_(True)
    # log p_t^θ(x ± v)
    logp_p = energy_net(x_plus, t)
    logp_m = energy_net(x_minus, t)
    # s^θ(x ± v) = ∇_x log p_t^θ(x ± v)
    score_plus = score_from_energy(logp_p, x_plus)
    score_minus = score_from_energy(logp_m, x_minus)
    # weak estimator of div_x s^θ(x,t)
    ...
    t_p = torch.clamp(t + h_d, 0.0, 1.0)
    t_m = torch.clamp(t - h_s, 0.0, 1.0)
    logp_t_p = energy_net(x_, t_p)
    logp_t = energy_net(x_, t)
...
def mb_light_fp_residual(energy_net: nn.Module, x: torch.Tensor, t: torch.Tensor, scheduler: nn.Module,
                         seed: int | None = None, h_s: float = 1e-3, h_d: float = 5e-4, ) -> torch.Tensor:
    """lightweight weak fokker–planck residual estimator (for mueller-brown experiment)
     using Hutchinson’s trace estimator for the score divergence"""
    if seed is not None:
        gen = torch.Generator(device=x.device)
        gen.manual_seed(seed)
    else:
